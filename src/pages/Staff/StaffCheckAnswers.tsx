...
const StaffCheckAnswers = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { toast } = useToast();
  const { placement, selectedDate, selectedTime, person } = location.state || {};

  const handleConfirmBooking = () => {
    toast({
      title: "Booking confirmed",
      description: "The placement has been successfully booked.",
    });
    navigate("/staff-booking-confirmation", { 
      state: { placement, selectedDate, selectedTime, person } 
    });
  };

  if (!placement || !selectedDate || !selectedTime || !person) {
...
export default StaffCheckAnswers;
